...
from diffusion_policy.real_world.urx_ros_controller import ROSURController
from diffusion_policy.real_world.multi_realsense_ros import ROSMultiRealsense, ROSRealsense
from diffusion_policy.common.timestamp_accumulator import (
    TimestampObsAccumulator, 
    TimestampActionAccumulator,
    align_timestamps
)
from diffusion_policy.real_world.multi_camera_visualizer import ROSMultiCameraVisualizer
...
class RealEnv:
    def __init__(self, 
        ...
        self.robot = ROSURController(
            ...
            tcp_offset_pose=[0, 0, tcp_offset, 0, 0, 0] if tcp_offset else None,
        ...
        color_transform = get_image_transform(
            ...
            bgr_to_rgb=True)
        if obs_float32:
            color_transform = lambda x: color_transform(x).astype(np.float32) / 255
        
        # Calculate visualization layout
        rw, rh, col, row = optimal_row_cols(
            n_cameras=len(camera_topics),
        ...
        if not record_raw_video:
            recording_transform = color_transform
        ...
        if enable_multi_cam_vis:
            self.multi_cam_vis = ROSMultiCameraVisualizer(
                realsense=self.realsense,
                row=row,
                col=col,
                window_name='ROS Multi Cam Vis',
                vis_fps=video_capture_fps,
                rgb_to_bgr=False
            )
